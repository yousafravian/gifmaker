...
export class FFmpegService {
  ...
  async loadFFmpeg() {
    try {
      ...
      FFmpegService.ffmpeg.on('log', e => console.log('LOG:', e.message, e.type));

      FFmpegService.ffmpeg.on('progress', e => console.log('Progress:', e.progress, e.time));
  ...
  async convertFile(file: File, format: string) {
    ...
    await FFmpegService.ffmpeg.exec(['-i', name, '-f', format, `${outputFileName}.${format}`]);
    // Read the result
    const data = await FFmpegService.ffmpeg.readFile(`${outputFileName}.${format}`);
